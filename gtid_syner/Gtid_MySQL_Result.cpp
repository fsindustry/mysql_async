...
namespace gtid_sync {
    ...
    bool Gtid_MySQL_Result::init(MYSQL *mysql, MYSQL_RES *res) {
    ...
    int Gtid_MySQL_Result::result_data(vec_row_t &data) {
        if (m_res == nullptr) {
            return 0;
        }
    ...
    MYSQL_ROW Gtid_MySQL_Result::fetch_row() {
        if (m_res == nullptr) {
            return nullptr;
        }
    ...
    unsigned int Gtid_MySQL_Result::num_rows() {
        if (m_res == nullptr) {
            return 0;
        }
    ...
    unsigned long *Gtid_MySQL_Result::fetch_lengths() {
        if (m_res == nullptr) {
            return nullptr;
        }
    ...
    unsigned int Gtid_MySQL_Result::fetch_num_fields() {
        if (m_res == nullptr) {
            return 0;
        }
